...
(function (jQuery, angular) {
    ...
    angular.module('Condo').
        ...
        factory('Condo.Amazon', ['$q', 'Condo.Md5', '$window', function ($q, md5, $window) {
            var PENDING = 0,
                STARTED = 1,
                PAUSED = 2,
                UPLOADING = 3,
                        ...
                        AmazonChunked = function (data, first_chunk) {
                            ...
                            var part_ids = [],
                                ...
                                generatePartManifest = function () {
                                    ...
                                    for (i = 0, length = part_ids.length; i < length; i += 1) {
                            ...
                            api.process_request(data).then(function (response) {
                                if (data.type === 'parts') {    // was the original request for a list of parts
                                    ...
                                    //
                                    response = jQuery(response[0]);
                                    var next = parseInt(response.find('NextPartNumberMarker').eq(0).text(), 10),
                                        etags = response.find('ETag');

                                    etags.each(function () {
                                        part_ids.push(jQuery(this).text().replace(/"{1}/gi, ''));    // Removes " from strings
                                    });
                                    ...
                                    //
                                    api.update({
                                        resumable_id: jQuery(response[0]).find('UploadId').eq(0).text(),
                                        file_id: $window.btoa(hexToBin(first_chunk.data_id)),
                                        part: 1
                                    }).then(function (data) {
        ...
        .run(['Condo.Amazon', 'Condo.Api', function (amazon, api) {
            api.register('AmazonS3', amazon);
        }]);
